 ...
 */
public abstract class AbstractSession implements com.arthenica.ffmpegkit.Session {
     ...
     */
    protected final com.arthenica.ffmpegkit.LogCallback logCallback;
     ...
     */
    protected final List<com.arthenica.ffmpegkit.Log> logs;
     ...
     */
    protected com.arthenica.ffmpegkit.SessionState state;

    /**
     * Return code for the completed sessions.
     */
    protected com.arthenica.ffmpegkit.ReturnCode returnCode;
...
//    protected String failStackTrace;
     ...
     */
    protected final com.arthenica.ffmpegkit.LogRedirectionStrategy logRedirectionStrategy;
    ...
    protected AbstractSession(final String[] arguments,
        ...
        this.future = null;
        this.state = com.arthenica.ffmpegkit.SessionState.CREATED;
        this.returnCode = null;
//        this.failStackTrace = null;
        this.logRedirectionStrategy = logRedirectionStrategy;

        com.arthenica.ffmpegkit.FFmpegKitConfig.addSession(this);
    }

    @Override
    public com.arthenica.ffmpegkit.LogCallback getLogCallback() {
        return logCallback;
    }
    ...
    public String getCommand() {
        return com.arthenica.ffmpegkit.FFmpegKitConfig.argumentsToString(arguments);
    }

    @Override
    public List<com.arthenica.ffmpegkit.Log> getAllLogs(final int waitTimeout) {
        waitForAsynchronousMessagesInTransmit(waitTimeout);

        if (thereAreAsynchronousMessagesInTransmit()) {
            android.util.Log.i(com.arthenica.ffmpegkit.FFmpegKitConfig.TAG, String.format("getAllLogs was called to return all logs but there are still logs being transmitted for session id %d.", sessionId));
        }
     ...
     */
    @Override
    public List<com.arthenica.ffmpegkit.Log> getAllLogs() {
        return getAllLogs(DEFAULT_TIMEOUT_FOR_ASYNCHRONOUS_MESSAGES_IN_TRANSMIT);
    }

    @Override
    public List<com.arthenica.ffmpegkit.Log> getLogs() {
    ...
    public String getAllLogsAsString(final int waitTimeout) {
        ...
        if (thereAreAsynchronousMessagesInTransmit()) {
            android.util.Log.i(com.arthenica.ffmpegkit.FFmpegKitConfig.TAG, String.format("getAllLogsAsString was called to return all logs but there are still logs being transmitted for session id %d.", sessionId));
        }
    ...
    @Override
    public com.arthenica.ffmpegkit.SessionState getState() {
        return state;
    }

    @Override
    public com.arthenica.ffmpegkit.ReturnCode getReturnCode() {
        return returnCode;
    }
...
//    }
    ...
    @Override
    public com.arthenica.ffmpegkit.LogRedirectionStrategy getLogRedirectionStrategy() {
        return logRedirectionStrategy;
    }

    @Override
    public boolean thereAreAsynchronousMessagesInTransmit() {
        return (com.arthenica.ffmpegkit.FFmpegKitConfig.messagesInTransmit(sessionId) != 0);
    }

    @Override
    public void addLog(final com.arthenica.ffmpegkit.Log log) {
        synchronized (logsLock) {
            this.logs.add(log);
        }
    }
    ...
    @Override
    public void cancel() {
        if (state == com.arthenica.ffmpegkit.SessionState.RUNNING) {
     ...
     */
    void startRunning() {
        this.state = com.arthenica.ffmpegkit.SessionState.RUNNING;
     ...
     */
    void complete(final com.arthenica.ffmpegkit.ReturnCode returnCode) {
        this.returnCode = returnCode;
        this.state = com.arthenica.ffmpegkit.SessionState.COMPLETED;
...
//        this.failStackTrace = Exceptions.getStackTraceString(exception);
        this.state = com.arthenica.ffmpegkit.SessionState.FAILED;
        this.endTime = new Date();
    }
