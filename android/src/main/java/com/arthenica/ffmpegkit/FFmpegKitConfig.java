...
public class FFmpegKitConfig {
    ...
    private static com.arthenica.ffmpegkit.Level activeLogLevel;

    /* Session history variables */
    private static int sessionHistorySize;
    private static final Map<Long, com.arthenica.ffmpegkit.Session> sessionHistoryMap;
    private static final List<com.arthenica.ffmpegkit.Session> sessionHistoryList;
    ...
    /* Global callbacks */
    private static com.arthenica.ffmpegkit.LogCallback globalLogCallback;
    private static com.arthenica.ffmpegkit.StatisticsCallback globalStatisticsCallback;
    private static com.arthenica.ffmpegkit.FFmpegSessionCompleteCallback globalFFmpegSessionCompleteCallback;
    private static com.arthenica.ffmpegkit.FFprobeSessionCompleteCallback globalFFprobeSessionCompleteCallback;
    private static com.arthenica.ffmpegkit.MediaInformationSessionCompleteCallback globalMediaInformationSessionCompleteCallback;
    ...
    private static com.arthenica.ffmpegkit.LogRedirectionStrategy globalLogRedirectionStrategy;
...
//        Exceptions.registerRootPackage("com.arthenica");
        ...
        final boolean nativeFFmpegTriedAndFailed = com.arthenica.ffmpegkit.NativeLoader.loadFFmpeg();

        /* ALL FFMPEG-KIT LIBRARIES LOADED AT STARTUP */
        com.arthenica.ffmpegkit.Abi.class.getName();
        com.arthenica.ffmpegkit.FFmpegKit.class.getName();
        com.arthenica.ffmpegkit.FFprobeKit.class.getName();

        com.arthenica.ffmpegkit.NativeLoader.loadFFmpegKit(nativeFFmpegTriedAndFailed);

        uniqueIdGenerator = new AtomicInteger(1);

        /* NATIVE LOG LEVEL IS RECEIVED ONLY ON STARTUP */
        activeLogLevel = com.arthenica.ffmpegkit.Level.from(com.arthenica.ffmpegkit.NativeLoader.loadLogLevel());

        asyncConcurrencyLimit = 10;
        asyncExecutorService = Executors.newFixedThreadPool(asyncConcurrencyLimit);

        sessionHistorySize = 10;
        sessionHistoryMap = new LinkedHashMap<Long, com.arthenica.ffmpegkit.Session>() {

            @Override
            protected boolean removeEldestEntry(Map.Entry<Long, com.arthenica.ffmpegkit.Session> eldest) {
        ...
        globalLogRedirectionStrategy = com.arthenica.ffmpegkit.LogRedirectionStrategy.PRINT_LOGS_WHEN_NO_CALLBACKS_DEFINED;

        android.util.Log.i(FFmpegKitConfig.TAG, String.format("Loaded ffmpeg-kit-%s-%s-%s-%s.", com.arthenica.ffmpegkit.NativeLoader.loadPackageName(), com.arthenica.ffmpegkit.NativeLoader.loadAbi(), com.arthenica.ffmpegkit.NativeLoader.loadVersion(), com.arthenica.ffmpegkit.NativeLoader.loadBuildDate()));
    }
    ...
    private static void log(final long sessionId, final int levelValue, final byte[] logMessage) {
        final com.arthenica.ffmpegkit.Level level = com.arthenica.ffmpegkit.Level.from(levelValue);
        final String text = new String(logMessage);
        final com.arthenica.ffmpegkit.Log log = new com.arthenica.ffmpegkit.Log(sessionId, level, text);
        boolean globalCallbackDefined = false;
        boolean sessionCallbackDefined = false;
        com.arthenica.ffmpegkit.LogRedirectionStrategy activeLogRedirectionStrategy = globalLogRedirectionStrategy;

        // AV_LOG_STDERR logs are always redirected
        if ((activeLogLevel == com.arthenica.ffmpegkit.Level.AV_LOG_QUIET && levelValue != com.arthenica.ffmpegkit.Level.AV_LOG_STDERR.getValue()) || levelValue > activeLogLevel.getValue()) {
            // LOG NEITHER PRINTED NOR FORWARDED
            return;
        }

        final com.arthenica.ffmpegkit.Session session = getSession(sessionId);
        if (session != null) {
...
//                    android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside session log callback.%s", Exceptions.getStackTraceString(e)));
        ...
        final com.arthenica.ffmpegkit.LogCallback globalLogCallbackFunction = FFmpegKitConfig.globalLogCallback;
...
//                android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside global log callback.%s", Exceptions.getStackTraceString(e)));
    ...
    private static void statistics(final long sessionId, final int videoFrameNumber,
        ...
        final com.arthenica.ffmpegkit.Statistics statistics = new com.arthenica.ffmpegkit.Statistics(sessionId, videoFrameNumber, videoFps, videoQuality, size, time, bitrate, speed);

        final com.arthenica.ffmpegkit.Session session = getSession(sessionId);
        if (session != null && session.isFFmpeg()) {
            com.arthenica.ffmpegkit.FFmpegSession ffmpegSession = (com.arthenica.ffmpegkit.FFmpegSession) session;
...
//                    android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside session statistics callback.%s", Exceptions.getStackTraceString(e)));
        ...
        final com.arthenica.ffmpegkit.StatisticsCallback globalStatisticsCallbackFunction = FFmpegKitConfig.globalStatisticsCallback;
...
//            android.util.Log.e(TAG, String.format("Failed to set font directory: %s.%s", Arrays.toString(fontDirectoryList.toArray()), Exceptions.getStackTraceString(e)));
    ...
    public static List<String> getSupportedCameraIds(final Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            detectedCameraIdList.addAll(com.arthenica.ffmpegkit.CameraSupport.extractSupportedCameraIds(context));
        }
     ...
     */
    public static void ignoreSignal(final com.arthenica.ffmpegkit.Signal signal) {
     ...
     */
    public static void ffmpegExecute(final com.arthenica.ffmpegkit.FFmpegSession ffmpegSession) {
        ffmpegSession.startRunning();

        try {
            final int returnCode = nativeFFmpegExecute(ffmpegSession.getSessionId(), ffmpegSession.getArguments());
            ffmpegSession.complete(new com.arthenica.ffmpegkit.ReturnCode(returnCode));
        } catch (final Exception e) {
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFmpeg execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffmpegSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     */
    public static void ffprobeExecute(final com.arthenica.ffmpegkit.FFprobeSession ffprobeSession) {
        ...
        try {
            ...
            ffprobeSession.complete(new com.arthenica.ffmpegkit.ReturnCode(returnCode));
        } catch (final Exception e) {
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFprobe execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffprobeSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     */
    public static void getMediaInformationExecute(final com.arthenica.ffmpegkit.MediaInformationSession mediaInformationSession, final int waitTimeout) {
        ...
        try {
            ...
            final com.arthenica.ffmpegkit.ReturnCode returnCode = new com.arthenica.ffmpegkit.ReturnCode(returnCodeValue);
            mediaInformationSession.complete(returnCode);
            if (((com.arthenica.ffmpegkit.ReturnCode) returnCode).isValueSuccess()) {
                List<com.arthenica.ffmpegkit.Log> allLogs = mediaInformationSession.getAllLogs(waitTimeout);
                final StringBuilder ffprobeJsonOutput = new StringBuilder();
                for (int i = 0, allLogsSize = allLogs.size(); i < allLogsSize; i++) {
                    com.arthenica.ffmpegkit.Log log = allLogs.get(i);
                    if (log.getLevel() == com.arthenica.ffmpegkit.Level.AV_LOG_STDERR) {
                        ffprobeJsonOutput.append(log.getMessage());
                    }
                }
                com.arthenica.ffmpegkit.MediaInformation mediaInformation = com.arthenica.ffmpegkit.MediaInformationJsonParser.fromWithError(ffprobeJsonOutput.toString());
...
//            android.util.Log.w(FFmpegKitConfig.TAG, String.format("Get media information execute failed: %s.%s", FFmpegKitConfig.argumentsToString(mediaInformationSession.getArguments()), Exceptions.getStackTraceString(e)));
     ...
     * @param ffmpegSession FFmpeg session which includes command options/arguments
     */
    public static void asyncFFmpegExecute(final com.arthenica.ffmpegkit.FFmpegSession ffmpegSession) {
        com.arthenica.ffmpegkit.AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new com.arthenica.ffmpegkit.AsyncFFmpegExecuteTask(ffmpegSession);
     ...
     */
    public static void asyncFFmpegExecute(final com.arthenica.ffmpegkit.FFmpegSession ffmpegSession, final ExecutorService executorService) {
        com.arthenica.ffmpegkit.AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new com.arthenica.ffmpegkit.AsyncFFmpegExecuteTask(ffmpegSession);
     ...
     * @param ffprobeSession FFprobe session which includes command options/arguments
     */
    public static void asyncFFprobeExecute(final com.arthenica.ffmpegkit.FFprobeSession ffprobeSession) {
        com.arthenica.ffmpegkit.AsyncFFprobeExecuteTask asyncFFmpegExecuteTask = new com.arthenica.ffmpegkit.AsyncFFprobeExecuteTask(ffprobeSession);
        Future<?> future = asyncExecutorService.submit(asyncFFmpegExecuteTask);
     ...
     * @param executorService executor service that will be used to run this asynchronous operation
     */
    public static void asyncFFprobeExecute(final com.arthenica.ffmpegkit.FFprobeSession ffprobeSession, final ExecutorService executorService) {
        com.arthenica.ffmpegkit.AsyncFFprobeExecuteTask asyncFFmpegExecuteTask = new com.arthenica.ffmpegkit.AsyncFFprobeExecuteTask(ffprobeSession);
        Future<?> future = executorService.submit(asyncFFmpegExecuteTask);
        ffprobeSession.setFuture(future);
    }
     ...
     * @param waitTimeout             max time to wait until media information is transmitted
     */
    public static void asyncGetMediaInformationExecute(final com.arthenica.ffmpegkit.MediaInformationSession mediaInformationSession, final int waitTimeout) {
        com.arthenica.ffmpegkit.AsyncGetMediaInformationTask asyncGetMediaInformationTask = new com.arthenica.ffmpegkit.AsyncGetMediaInformationTask(mediaInformationSession, waitTimeout);
     ...
     * @param waitTimeout             max time to wait until media information is transmitted
     */
    public static void asyncGetMediaInformationExecute(final com.arthenica.ffmpegkit.MediaInformationSession mediaInformationSession, final ExecutorService executorService, final int waitTimeout) {
        com.arthenica.ffmpegkit.AsyncGetMediaInformationTask asyncGetMediaInformationTask = new com.arthenica.ffmpegkit.AsyncGetMediaInformationTask(mediaInformationSession, waitTimeout);
     ...
     */
    public static void enableLogCallback(final com.arthenica.ffmpegkit.LogCallback logCallback) {
     ...
     *                           defined callback
     */
    public static void enableStatisticsCallback(final com.arthenica.ffmpegkit.StatisticsCallback statisticsCallback) {
     ...
     */
    public static void enableFFmpegSessionCompleteCallback(final com.arthenica.ffmpegkit.FFmpegSessionCompleteCallback ffmpegSessionCompleteCallback) {
     ...
     */
    public static com.arthenica.ffmpegkit.FFmpegSessionCompleteCallback getFFmpegSessionCompleteCallback() {
     ...
     *                                       previously defined callback
     */
    public static void enableFFprobeSessionCompleteCallback(final com.arthenica.ffmpegkit.FFprobeSessionCompleteCallback ffprobeSessionCompleteCallback) {
     ...
     */
    public static com.arthenica.ffmpegkit.FFprobeSessionCompleteCallback getFFprobeSessionCompleteCallback() {
     ...
     */
    public static void enableMediaInformationSessionCompleteCallback(final com.arthenica.ffmpegkit.MediaInformationSessionCompleteCallback mediaInformationSessionCompleteCallback) {
     ...
     */
    public static com.arthenica.ffmpegkit.MediaInformationSessionCompleteCallback getMediaInformationSessionCompleteCallback() {
     ...
     */
    public static com.arthenica.ffmpegkit.Level getLogLevel() {
     ...
     */
    public static void setLogLevel(final com.arthenica.ffmpegkit.Level level) {
        if (level != null) {
...
//            android.util.Log.e(TAG, String.format("Failed to close SAF fd: %d.%s", fileDescriptor, Exceptions.getStackTraceString(t)));
    ...
    private static void deleteExpiredSessions() {
        while (sessionHistoryList.size() > sessionHistorySize) {
            try {
                com.arthenica.ffmpegkit.Session expiredSession = sessionHistoryList.remove(0);
     ...
     */
    static void addSession(final com.arthenica.ffmpegkit.Session session) {
     ...
     */
    public static com.arthenica.ffmpegkit.Session getSession(final long sessionId) {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static com.arthenica.ffmpegkit.Session getLastSession() {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static com.arthenica.ffmpegkit.Session getLastCompletedSession() {
        synchronized (sessionHistoryLock) {
            for (int i = sessionHistoryList.size() - 1; i >= 0; i--) {
                final com.arthenica.ffmpegkit.Session session = sessionHistoryList.get(i);
     ...
     */
    public static List<com.arthenica.ffmpegkit.Session> getSessions() {
        synchronized (sessionHistoryLock) {
     ...
     */
    public static List<com.arthenica.ffmpegkit.FFmpegSession> getFFmpegSessions() {
        final LinkedList<com.arthenica.ffmpegkit.FFmpegSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.arthenica.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isFFmpeg()) {
                    list.add((com.arthenica.ffmpegkit.FFmpegSession) session);
                }
            }
        }
     ...
     */
    public static List<com.arthenica.ffmpegkit.FFprobeSession> getFFprobeSessions() {
        final LinkedList<com.arthenica.ffmpegkit.FFprobeSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.arthenica.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isFFprobe()) {
                    list.add((com.arthenica.ffmpegkit.FFprobeSession) session);
                }
            }
        }
     ...
     */
    public static List<com.arthenica.ffmpegkit.MediaInformationSession> getMediaInformationSessions() {
        final LinkedList<com.arthenica.ffmpegkit.MediaInformationSession> list = new LinkedList<>();

        synchronized (sessionHistoryLock) {
            for (com.arthenica.ffmpegkit.Session session : sessionHistoryList) {
                if (session.isMediaInformation()) {
                    list.add((com.arthenica.ffmpegkit.MediaInformationSession) session);
                }
            }
        }
     ...
     */
    public static List<com.arthenica.ffmpegkit.Session> getSessionsByState(final com.arthenica.ffmpegkit.SessionState state) {
        final LinkedList<com.arthenica.ffmpegkit.Session> list = new LinkedList<>();
     ...
     */
    public static com.arthenica.ffmpegkit.LogRedirectionStrategy getLogRedirectionStrategy() {
     ...
     */
    public static void setLogRedirectionStrategy(final com.arthenica.ffmpegkit.LogRedirectionStrategy logRedirectionStrategy) {
     ...
     * @return string value
     */
    public static String sessionStateToString(final com.arthenica.ffmpegkit.SessionState state) {
